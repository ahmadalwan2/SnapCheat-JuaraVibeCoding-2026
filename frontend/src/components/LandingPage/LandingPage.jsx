    ...
    <>
      ...
      <nav className="fixed w-full top-0 z-50 bg-[#fafafa]/80 backdrop-blur-xl border-b border-[#eaeaea]">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <a href="#hero" className="flex items-center gap-1.5 cursor-pointer hover:opacity-80 transition-opacity">
            <img 
            ...
            />
            <span className="text-xl font-bold font-heading tracking-normal">
              Snapcheat
            </span>
          </a>
          ...
          <div className="hidden md:flex items-center gap-4">
            {user ? (
              <div className="flex items-center gap-4">
                ...
                <button onClick={() => navigate('/dashboard')} className="bg-[#2FA084] hover:bg-[#258069] text-white px-5 py-2 rounded-xl text-sm font-medium transition-colors cursor-pointer">
                  Buka Dashboard
                </button>
              </div>
            ) : (
              <>
                <button onClick={() => navigate('/login')} className="text-sm font-medium text-[#666666] hover:text-[#171717] transition-colors cursor-pointer">Masuk</button>
                <button onClick={() => navigate('/register')} className="bg-[#2FA084] hover:bg-[#258069] text-white px-5 py-2 rounded-xl text-sm font-medium transition-colors cursor-pointer">
                  Mulai Gratis
                </button>
              </>
            )}
            ...
            <div className="flex flex-col gap-3 pt-4 border-t border-[#eaeaea]">
              {user ? (
                <>
                  ...
                  <button onClick={() => { navigate('/dashboard'); setIsMobileMenuOpen(false); }} className="w-full bg-[#2FA084] hover:bg-[#258069] text-white px-5 py-3 rounded-xl text-sm font-medium transition-colors cursor-pointer">
                    Buka Dashboard
                  </button>
                </>
              ) : (
                <>
                  <button onClick={() => { navigate('/login'); setIsMobileMenuOpen(false); }} className="text-center py-2 text-sm font-medium text-[#666666] hover:text-[#171717] cursor-pointer">Masuk</button>
                  <button onClick={() => { navigate('/register'); setIsMobileMenuOpen(false); }} className="w-full bg-[#2FA084] hover:bg-[#258069] text-white px-5 py-3 rounded-xl text-sm font-medium transition-colors cursor-pointer">
                    Mulai Gratis
                  </button>
                </>
              )}
      ...
      <section id="hero" className="pt-32 pb-20 px-6 relative flex flex-col items-center text-center">
        ...
        <div className="max-w-4xl mx-auto" data-aos="fade-up">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16" data-aos="fade-up" data-aos-delay="400">
            {!user && (
              <button onClick={() => navigate('/register')} className="w-full sm:w-auto bg-[#2FA084] hover:bg-[#258069] text-white px-8 py-3.5 rounded-xl font-medium transition-colors flex items-center justify-center gap-2 cursor-pointer">
      ...
      {/* Floating Chatbot Widget (Hanya di Landing Page) */}
      <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 flex flex-col items-end gap-4">
        ...
        {isChatOpen && (
          <div className="w-[calc(100vw-2rem)] sm:w-96 bg-white border border-[#eaeaea] rounded-2xl shadow-2xl overflow-hidden flex flex-col origin-bottom-right transition-all duration-300">
            ...
            {/* Chat Messages */}
            <div className="p-4 h-64 overflow-y-auto overflow-x-hidden bg-[#fafafa] flex flex-col gap-4">
              {chatMessages.map((msg, idx) => (
                <div key={idx} className={`flex items-start gap-2 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`p-3 rounded-2xl text-sm shadow-sm whitespace-pre-wrap leading-relaxed break-words max-w-[80%] ${msg.role === 'user' ? 'bg-[#2FA084] text-white rounded-tr-none' : 'bg-white border border-[#eaeaea] text-[#666666] rounded-tl-none'}`}>
